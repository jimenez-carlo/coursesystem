    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="/coursesystem/pages/semester/semester.php" class="nav-link active">
            <i class="nav-icon fas fa-user"></i>
              <p>
                Semester
              </p>
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 style="font-weight: bold;">MANAGE SEMESTER</h1 style>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>
            ...
            <div class="card-header">
                <div class="row align-items-center">
                    ...
                    <div class="input-group input-group-sm" style="width: 150px;">
                            ...
                            <!-- Modal to display semester details -->
<div class="modal" id="semesterModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <!-- Modal Body -->
                <div class="modal-body" id="semesterDetails">
                    <!-- Semester details will be displayed here -->
                </div>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                <td>" . $row["semester"] . "</td>
                <td>" . $row["sem_status"] . "</td>"; // Added this line to display sem_status

        // Display button for Edit and Delete
        echo "<td>
                <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#modal-edit' data-semester-id='" . $row['sem_id'] . "'>
                    <i class='fas fa-edit'></i>
                </button>
                <button type='button' class='btn btn-danger' onclick='confirmDelete(" . $row['sem_id'] . ")'>
        ...
        if ($row["sem_status"] == 'Active') {
            echo "<button class='btn btn-success' onclick='deactivateSemester(" . $row['sem_id'] . ")'>Active</button>";
        } else {
            echo "<button class='btn btn-danger' onclick='activateSemester(" . $row['sem_id'] . ")'>Deactivated</button>";
        }
        echo "</td></tr>";
    }
} else {
...
<div class="modal fade" id="modal-add" tabindex="-1" role="dialog" aria-labelledby="modal-add-title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="add_semester.php" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
...
<div class="modal fade" id="modal-edit" tabindex="-1" role="dialog" aria-labelledby="modal-edit-title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="edit_semester.php" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
...
function confirmDelete(semesterId) {
        swal({
            ...
            if (willDelete) {
                ...
                swal("Poof! Semester has been deleted!", {
                    icon: "success",
                }).then(() => {
                    // Redirect to the delete script with the user ID
                    window.location.href = 'delete_semester.php?sem_id=' + semesterId;
                });
            } else {
  ...
  // Script to handle opening the edit modal and passing the sem_id
$(document).ready(function() {
    $('#modal-edit').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget); // Button that triggered the modal
        var semesterId = button.data('semester-id'); // Extract sem_id from data-attribute
        var modal = $(this);
        modal.find('.modal-body #sem_id').val(semesterId); // Set the sem_id in the modal form
    });
});

    function deactivateSemester(semesterId) {
        // Implement your deactivate logic, e.g., update user_account_status to 'Deactivated'
        if (confirm("Are you sure you want to deactivate this semester?")) {
            // Make an AJAX request or redirect to a PHP script to handle deactivation
            window.location.href = 'deactivate_semester.php?id=' + semesterId;
        }
    }

    function activateSemester(semesterId) {
        // Implement your activate logic, e.g., update user_account_status to 'Active'
        if (confirm("Are you sure you want to activate this semester?")) {
            // Make an AJAX request or redirect to a PHP script to handle activation
            window.location.href = 'activate_semester.php?id=' + semesterId;
        }
    }
       ...
       $(document).ready(function() {
            ...
            $('#searchResults').on('click', 'li', function() {
                var semesterName = $(this).text();
                // Open the modal
                $('#semesterModal').modal('show');
                // Fetch and display semester details in the modal
                $.ajax({
                    type: 'POST',
                    url: 'get_semester_details.php', // PHP file to handle the AJAX request
                    data: { semestertName: semesterName },
                    success: function(response) {
                        $('#semestertDetails').html(response);
                    }
                });
            });
        });
