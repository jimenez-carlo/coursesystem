  ...
  <div class="modal-body">
      ...
      <div class="form-group">
        ...
        <select name="department_id" id="department_id" class="form-control">
          <?php foreach (get_list("SELECT * from department_tbl where deleted_flag = 0") as $row) { ?>
            <option value="<?= $row['department_id'] ?>" <?= $row['department_id'] == $data->access_id ? "selected" : "" ?>><?= $row['department_code'] . " (" . $row['department_title'] . ")" ?></option>
          <?php } ?>
        </select>
      </div>
    </div>
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        <input required type="text" class="form-control" id="admin_firstname" name="admin_firstname" value="<?= $data->admin_firstname ?>" required>
      </div>
      <div class="form-group col-md-6">
        <label for="admin_lastname" class="font-weight-bold">Last Name:</label>
        <input required type="text" class="form-control" id="admin_lastname" name="admin_lastname" value="<?= $data->admin_lastname ?>" required>
      </div>
    </div>
    <div class="form-group">
      <div class="form-group">
