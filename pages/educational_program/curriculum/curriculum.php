              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    ...
                    $sql = "SELECT c.*,d.department_course,ys.* FROM curriculum_tbl c inner join department_tbl d on d.department_id = c.department_id inner join year_semester_tbl ys on ys.year_semester_id = c.year_semester_id";
                ...
                <td>" . $row["year_from"] . " - " . $row["year_to"] . "</td> ";
                        ...
                        echo "<td>
  <a class='btn btn-success' href=curriculum_subjects.php?id=" . $row['curriculum_id'] . "><i class='fas fa-book'></i></a>
  <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#modal-edit' data-curriculum-id='" . $row['curriculum_id'] . "' data-department-id='" . $row['department_id'] . "' data-year-semester-id='" . $row['year_semester_id'] . "' data-curriculum-name='" . $row['curriculum_name'] . "' data-description='" . $row['description'] . "'><i class='fas fa-edit'></i></button>
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="year_semester_id" name="year_semester_id" required>
                  <option value="">Select Year</option>
                  <?php
                  include('../../../conn.php');

                  // Fetch data from curriculum_tbl
                  $sql = "SELECT * FROM year_semester_tbl";
                  $result = $conn->query($sql);
                  ...
                  if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) { ?>
                      ...
                      <option value="<?= $row['year_semester_id'] ?>"><?= $row['year_from'] . " - " . $row['year_to'] ?></option>
                  <?php
                    }
                  }
              ...
              <label for="curriculum_name">Description:</label>
              <textarea name="description" id="description" class="form-control" row="6"></textarea>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
